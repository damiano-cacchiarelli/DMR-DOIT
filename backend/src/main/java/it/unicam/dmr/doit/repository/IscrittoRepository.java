...
public interface IscrittoRepository<I extends Iscritto> extends JpaRepository<I, String> {
	...
	/**
	 * @return tutti gli iscritti aventi il ruolo esperto ordinati in modo decrescente in base al rango
	 */
	...
	/**
	 * @return tutti gli iscritti aventi il ruolo esperto e che hanno valutato almeno un progetto che presenta la lista dei tag passati
	 */
	//@Query("SELECT i FROM Iscritto i JOIN Ruolo r ON r.iscritto_identificativo = i.identificativo JOIN Valutazione v ON v.id_esperto = r.id JOIN Progetto p ON p.id = v.id_progetto JOIN categorie_progetti cp ON cp.progetto = p.id WHERE r.ruolo = 'ROLE_ESPERTO' AND cp.categoria IN ?1")
